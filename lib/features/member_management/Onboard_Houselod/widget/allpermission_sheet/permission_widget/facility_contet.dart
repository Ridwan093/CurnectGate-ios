...
import 'package:curnectgate/features/member_management/Onboard_Houselod/widget/allpermission_sheet/permission_widget/facilityMutleSelection.dart';
...
class FacilityContet extends ConsumerStatefulWidget {
  final Map<String, Permission>? permission;
  final int id;
  const FacilityContet({super.key, required this.permission, required this.id});

  @override
  ConsumerState<FacilityContet> createState() => _FacilityContetState();
}

class _FacilityContetState extends ConsumerState<FacilityContet> {
  ...
  Permission? get facilityAccess => widget.permission?['facility_access'];

  bool? get isFacilityEnabled =>
      ref.watch(notificationProviders).isFacilityEnabled;

  List<String>? get facilities => ref.watch(notificationProviders).facilities;
  ...
  void _initializeState() {
    ...
    final facilityAccess = widget.permission?['facility_access'];

    if (facilityAccess != null) {
      final conditions = facilityAccess.conditions;
      log("Not Null And work fine");

      notifier.updateFacilityAccess(
        isActive: facilityAccess.isActive ?? false,
            ...
            child: Column(
              children: [
                    ...
                    children: [
                      ...
                      Switch(
                        ...
                        onChanged: (value) {
                          notifier.updateFacilityAccess(
                            isActive: value,
                            conditions: facilityAccess?.conditions,
                          );
                        },
                      ),
                    ],
                ...
                ReusableReasonWidget(
                  ...
                  onChanged: (value) {
                    notifier.updateFacilityReason(value);
                  },
                  isEnable: !isFacilityEnabled!,
                ),
                const SizedBox(height: 16),
                ...
                // Facility Selector
                FacilitySelector(isFacilityEnabled),
  ...
  List<Widget> _buildDayTimePickers(BuildContext context, WidgetRef ref) {
    ...
    final timeRestrictions = state.facilityConditions?.timeRestrictions;
                  ...
                  children: [
                    ...
                    Timeboxe(
                      ...
                      onTimeSelected: (newTime) {
                        notifier.updateFacilityTimeRestriction(
                          day: dayKey,
                          startTime: newTime,
                          endTime: endTime,
                        );
