...
class PackingAccessContent extends ConsumerStatefulWidget {
  final Map<String, Permission>? permission;
  final int id;
  const PackingAccessContent({
    super.key,
    required this.id,
...
class _PackingAccessContentState extends ConsumerState<PackingAccessContent> {
  final _rasoncontoller = TextEditingController();
  final _max1Controller = TextEditingController();
  Permission? get packing => widget.permission?['visitor_invitation'];

  bool? get ispackingEnabled =>
      ref.watch(notificationProviders).ispackingEnabled;
  ...
  void _initializeState() {
    ...
    final paackingAccess = widget.permission?['parking_access'];

    if (paackingAccess != null) {
      notifier.togglePackingaccess(paackingAccess.isActive ?? false);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isLoading = ref.watch(formProvider).parkingAccessLoading;
