...
import 'package:curnectgate/features/member_management/Onboard_Houselod/widget/allpermission_sheet/permission_widget/nightAccess_content.dart';
import 'package:dio/dio.dart';
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';

class NightAccessSettingsScreen extends ConsumerWidget {
  final int id;

  const NightAccessSettingsScreen({super.key, required this.id});
        ...
        data: (profile) {
          try {
            ...
            if (user != null && user.permissions != null) {
              return NightAccessContent(permission: user.permissions, id: id);
            } else {
              return EmptyBodys(message: "No night settings?");
            }
          } catch (e) {
            return Builderroul(
              error: e.toString(),
              onTap:
                  () => ref
                      .read(statisticProvider.notifier)
                      .refreshPermission(context, ref),
              firstMessae: "Faile to load night settings?",
            );
          }
        },
        loading: () {
