...
final getCurfewSettingProvider =
    AsyncNotifierProvider.autoDispose<CurfewNotifer, CurfewResponse?>(() {
      return CurfewNotifer();
    });

class CurfewNotifer extends AutoDisposeAsyncNotifier<CurfewResponse?> {
  @override
  Future<CurfewResponse?> build() async {
    // First try to load from local storage
    final localPermission = await SharedPrefsService.getPermissionCurfew();
    ...
    try {
      ...
      final freshPermission = await ref
          ...
          .getPermissionCurfew(bearerToken: token, id: id);

      // Only update local storage if data is different
      if (localPermission?.toJson() != freshPermission.toJson()) {
        await SharedPrefsService.savePermissionCurfew(freshPermission);
      }
