...
class GrantEventEntry extends ConsumerStatefulWidget {
  final String rawMap;
  final int eventId;

  const GrantEventEntry({
    super.key,
  ...
  @override
  ConsumerState<GrantEventEntry> createState() => _GrantEventEntryState();
}

class _GrantEventEntryState extends ConsumerState<GrantEventEntry> {
  ...
  void _validateForm() {
    ...
    _isFormValid.value = name.length > 1 && phone.length > 9;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final extractedData = json.decode(widget.rawMap);
    final data = extractedData["data"] ?? {};
    final eventCode = data["event_code"] ?? {};
    final description =
        eventCode["event_description"] ?? "No description provided";
    final location = eventCode["event_location"] ?? "No location provided";
          ...
          child: Column(
            ...
            children: [
              ...
              ReusabelProfileForm(
                ...
                fieldType: FieldType.name, // Using name for flexible short text
