...
class ValidatedEventCode extends ConsumerStatefulWidget {
  ValidatedEventCode({super.key});

  @override
  ConsumerState<ValidatedEventCode> createState() =>
      _ValidateWorkOrderOtpState();
}

class _ValidateWorkOrderOtpState extends ConsumerState<ValidatedEventCode> {
  ...
  Widget build(BuildContext context) {
    ...
    final notifiers = ref.watch(oTpformProvider);
    return PopScope(
      canPop: true,
      onPopInvoked: (pop) {
        if (!pop) {
    ...
    return ActionSecurityButton(
      ...
      onPressed:
          formState.otpallValid
              ? () {
                providerState.validateEventCode(
                  context: context,
