...
class Validateotp extends ConsumerStatefulWidget {
  Validateotp({super.key});

  @override
  ConsumerState<Validateotp> createState() => _ValidateotpState();
}

class _ValidateotpState extends ConsumerState<Validateotp> {
  ...
  final otpConttoller = TextEditingController();
  ...
  Widget _buildBottomAction(WidgetRef ref, BuildContext context) {
    ...
    return ActionSecurityButton(
      ...
      onPressed:
          formState.allValid
              ? () {
                providerState.validateDigitalOTP(
                  context: context,
                  qrCodeData: formState.otpCode ?? "",
