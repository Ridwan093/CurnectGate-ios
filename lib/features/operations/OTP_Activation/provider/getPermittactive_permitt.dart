...
final getActivePermit =
    AsyncNotifierProvider.autoDispose<WorkPermitNotifier, ActiveOtpResponse?>(
      () {
        return WorkPermitNotifier();
      },
    );

class WorkPermitNotifier extends AutoDisposeAsyncNotifier<ActiveOtpResponse?> {
  @override
  Future<ActiveOtpResponse?> build() async {
    // First try to load from local storage
    final localVendor = await SharedPrefsService.getActivePermit();
    ...
    try {
      ...
      final freshVendor =
          await ref.read(getApiServiceProvider).geActivePermit();

      // Only update local storage if data is different
      if (localVendor?.toJson() != freshVendor.toJson()) {
        await SharedPrefsService.saveActivePermit(freshVendor);
      }
