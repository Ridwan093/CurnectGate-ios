...
final candidateResultProvider =
    AsyncNotifierProvider.autoDispose<CandidateNotifier, LiveResultsResponse?>(
      () {
        return CandidateNotifier();
      },
    );

class CandidateNotifier extends AutoDisposeAsyncNotifier<LiveResultsResponse?> {
  @override
  Future<LiveResultsResponse?> build() async {
    // First try to load from local storage
    final locaCandidate = await SharedPrefsService.getCandidateResult();
    ...
    try {
      ...
      final freshCanidate = await ref
          ...
          .getCandidateResult(bearerToken: token, id: id);

      // Only update local storage if data is different
      if (locaCandidate?.toJson() != freshCanidate.toJson()) {
        await SharedPrefsService.saveCandidateResult(freshCanidate);
      }
    ...
    state = await AsyncValue.guard(() async {
      try {
        ...
        final freshCandidate = await ref
            .read(getApiServiceProvider)
            .getCandidateResult(bearerToken: token!, id: id);
        await SharedPrefsService.saveCandidateResult(freshCandidate);
        ...
        final localcandidate = await SharedPrefsService.getCandidateResult();
